...
poly_pieces = {'bp': 0, 'wp': 1, 'bn': 2, 'wn': 3, 'bb': 4, 'wb': 5,
               'br': 6, 'wr': 7, 'bq': 8, 'wq': 9, 'bk': 10, 'wk': 11}

poly_rows = {0: 7, 1: 6, 2: 5, 3: 4, 4: 3, 5: 2, 6: 1, 7: 0}


def init_poly_book():
    file = r'C:\Users\45665\Desktop\Schoolwork\Coding\Python\MiniGames\ChessEngine2\fruit.bin'

    data_file = []

    file_fmt = '=QHHI'
    struct_size = struct.calcsize(file_fmt)
    struct_unpack = struct.Struct(file_fmt).unpack_from
    ...
    with open(file, "rb") as f:
        while True:
            ...
            data_file.append((hex(s[0]), s[1], s[2], s[3]))

    for row in data_file:
        if row[0] == hex(0x463b96181691fc9c):
            print(row)
        else:
            print(row)
...
def en_passant_move_possible(gs, pos):
    x, y = pos
    print(x, y)
    if not gs.white_to_move:
        if gs.board[x - 1][y + 1] == 'bp':
            print('bp')
            return y
        elif gs.board[x - 1][y - 1] == 'bp':
            print('bp')
            return y
    else:
        if gs.board[x + 1][y + 1] == 'wp':
            print('wp')
            return y
        elif gs.board[x + 1][y - 1] == 'wp':
            print('wp')
            return y


def poly_key_from_board(gs):
    final_key = 0
    piece = None
    poly_piece = 0
    offset = 0
    ...
    # pieces
    for i in range(0, 64):
        piece, row, col, notation = gs.board_square_to_pos(i)
        if piece != '--':
            poly_piece = poly_pieces[piece]
            row = poly_rows[row]
            final_piece ^= RANDOM64[64 * poly_piece + 8 * row + col]

    # castling
    offset = 768
    if gs.castle_rights.wks:
        final_castle ^= RANDOM64[offset]
    if gs.castle_rights.wqs:
        final_castle ^= RANDOM64[offset + 1]
    if gs.castle_rights.bks:
        final_castle ^= RANDOM64[offset + 2]
    if gs.castle_rights.bqs:
        final_castle ^= RANDOM64[offset + 3]

    # en passant
    offset = 772
    pos = gs.en_passant_possible
    if pos:
        positions = en_passant_move_possible(gs, pos)
        if positions:
            print(positions)
            final_en_passant ^= RANDOM64[positions + offset]

    # turn
    if gs.white_to_move:
        final_colour ^= RANDOM64[780]

    final_key = hex(final_piece ^ final_castle ^ final_en_passant ^ final_colour)

    while len(final_key) < 18:
        final_key = final_key[:2] + '0' + final_key[2:]

    return final_key


def get_book_move(board):
    poly_key = poly_key_from_board(board)
    print(poly_key)


def list_book_moves(poly_key, entries):
    start = 0
    move = 0

    for entry in entries:
        if poly_key == entry.key:
            move = entry.move
            print(move)
